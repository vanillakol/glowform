                    ...
                    <div>
                        ...
                        <div class="video-info">
                            <div class="rating-container">
                                <div class="rating-box">
                                    <h4>IMDb RATING</h4>
                                    <div class="value">${result.averageRating}/10</div>
                                </div>
                                <div class="rating-box">
                                    <h4>VOTES</h4>
                                    <div class="value">${result.numVotes.toLocaleString()}</div>
                                </div>
                                <div class="rating-box">
                                    <h4>CONTENT RATING</h4>
                                    <div class="value">${result.contentRating}</div>
                                </div>
                            </div>
    
                            <div class="meta-info">
                                <span>${result.startYear}${result.endYear ? ` - ${result.endYear}` : ''}</span>
                                <span>${result.totalSeasons} Seasons</span>
                                <span>${result.totalEpisodes} Episodes</span>
                                <span>${result.countriesOfOrigin.join(', ')}</span>
                                <span>${result.spokenLanguages.join(', ')}</span>
                            </div>
    
                            <h3>Genres</h3>
                            <div class="genres-container">
                                ${result.genres.map(genre => `<span class="genre-tag">${genre}</span>`).join('')}
                            </div>
                            ...
                            <div class="scroll-container">
                                ${result.cast.map(actor => `
                                    <div class="person-card">
                                        ...
                                        <p>${actor.job.charAt(0).toUpperCase() + actor.job.slice(1)}</p>
                                    </div>
                                `).join('')}
                            </div>
                        </div>
...
app.get('/search/:query', async (req, res) => {
    ...
    const url = `https://imdb236.p.rapidapi.com/imdb/search?primaryTitle=${query}&sortField=startYear`;
    const options = {
        method: 'GET',
        headers: {
            'x-rapidapi-key': 'c184b7c08dmshcb3d53a5367e794p14b044jsna1831453c2e6',
